...
## GSD Project Guidance

Read `.planning/PROJECT.md`, `.planning/REQUIREMENTS.md`, `.planning/ROADMAP.md`, and `.planning/STATE.md` for current scope and position.

The user requires a lean challenge submission. Use the existing terminal entry points and ordinary Python functions. A bounded model-driven request orchestrator with allowlisted evidence and deterministic financial tools is core; the model chooses workflow actions, while host code owns validation, money, safety and termination. Do not replace this with an extraction-only fixed pipeline or add a multi-agent framework. Process the supplied messages/images as financial inputs; do not add messaging, translation, language settings, a document platform, persistent caching/provenance infrastructure, or a usage-ledger service. Actual final-run token/cost totals and financial safety checks remain mandatory.

Use `$gsd-plan-phase` / `$gsd-execute-phase` for planned work and `$gsd-quick` for small changes. Keep planning state in sync. Automatic sequential work, plan checks, and verification remain enabled; routine research and separate UI/AI design contracts are disabled. Read technical documentation only when a concrete implementation question requires it.
