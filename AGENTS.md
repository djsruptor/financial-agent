...
The user requires a lean challenge submission. Use the existing terminal entry points and ordinary Python functions. Process the supplied messages/images as financial inputs; do not add messaging, translation, language settings, a document platform, persistent caching/provenance infrastructure, or a usage-ledger service. Actual final-run token/cost totals and financial safety checks remain mandatory.
